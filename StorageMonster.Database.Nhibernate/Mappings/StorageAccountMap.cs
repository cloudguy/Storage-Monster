...
namespace StorageMonster.Database.Nhibernate.Mappings
{
    public class StorageAccountMap : ClassMap<StorageAccount>
    {
        public StorageAccountMap()
        {
            ...
            References(x => x.User).Column("user_id").LazyLoad(Laziness.Proxy).Not.Nullable().UniqueKey("un_saccountlogin_spluginid");
            References(x => x.StoragePlugin).Column("storage_plugin_id").Not.Nullable().LazyLoad(Laziness.Proxy).UniqueKey("un_saccountlogin_spluginid");
