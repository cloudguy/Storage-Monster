...
#pragma warning restore 618
		...
		public static Func<IDataReader, object> GetTypeDeserializer(Type type, IDataReader reader, int startBound, int length, bool returnNullIfFirstMissing)
		{
			...
			List<Action<IDataReader, int ,object>> setterActions = new List<Action<IDataReader, int ,object>>();

            foreach (var setter in setters)
            {
                var setterLocal = setter;

                Action<IDataReader, int, object> action = (actionReader, propIndex, destObject) =>
                    {
                        var p = actionReader[propIndex];
                        if (p is DBNull)
                            return;
                        if ((setterLocal.Property != null && setterLocal.Property.Type.IsEnum)
                            || (setterLocal.Field != null && setterLocal.Field.FieldType.IsEnum))
                        {
                            string value;
                        ...
                        else if ((setterLocal.Property != null && setterLocal.Property.Type.Equals(typeof(DateTime)))
                            || (setterLocal.Field != null && setterLocal.Field.FieldType.Equals(typeof(DateTime)))
                            || (setterLocal.Property != null && setterLocal.Property.Type.Equals(typeof(DateTime?)))
                            ||setterLocal.Field != null && setterLocal.Field.FieldType.Equals(typeof(DateTime?)))
                        {
			...
			Func<IDataReader, object> func = (dataReader) =>
			{
				...
				{
                    SqlMapper.ThrowDataException(ex, index, dataReader);
				}
